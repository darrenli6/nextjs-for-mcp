...
export default async function AuthPage({ searchParams }: AuthPageProps) {
  ...
  const clientName = params.client_name || 'MCP client';
  const loginSucceeded = params.login === 'success';
      ...
      <section className="w-full max-w-md rounded-2xl border border-white/10 bg-white/10 p-8 shadow-2xl backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-semibold">
            {state || !loginSucceeded ? 'Sign in to continue' : 'Google login successful'}
          </h1>
          <p className="mt-3 text-sm leading-6 text-slate-300">
            {loginSucceeded
              ? `You are signed in${params.email ? ` as ${params.email}` : ''}.`
              : state
              ? `${clientName} is requesting access to this MCP server. Sign in with Google to authorize the connection.`
              : 'Sign in with Google to test this web login, or open this page from an MCP client authorization request.'}
          </p>
        </div>

        {loginSucceeded ? (
          <div className="rounded-xl border border-emerald-300/30 bg-emerald-300/10 px-4 py-3 text-center text-sm text-emerald-100">
            Your Google account has been saved to the database.
          </div>
        ) : state ? (
          <a
            href={`/api/oauth/google/start?state=${encodeURIComponent(state)}`}
